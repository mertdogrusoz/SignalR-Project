...
namespace SignalRWebUI.Controllers
{
	public class MenuTableController : Controller
	{
		...
		public async Task<IActionResult>Index()
		{
			...
			var responseMessage = await client.GetAsync("https://localhost:7032/api/MenuTable/GetMenuTable");
		...
		public async Task<IActionResult> CreateMenuTable(CreateMenuTableDto createMenuTableDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7032/api/MenuTable",stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"https://localhost:7032/api/MenuTable/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync($"https://localhost:7032/api/MenuTable/{id}");
		...
		public async Task<IActionResult> UpdateMenuTable(UpdateMenuTableDto updateMenuTableDto)
		{
			...
			var responseMessage = await client.PutAsync($"https://localhost:7032/api/MenuTable/", content);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
